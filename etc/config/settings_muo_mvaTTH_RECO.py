...
flags = {
   ...
   #'passtight'    : '(Probe_conept > 10 && Probe_jetBTagDeepFlavB < 0.2431 && Probe_mvaTTHRun3 > 0.64  && Probe_mediumId==1 &&  Probe_tightCharge == 2)',
   #'passtight'   : '(Probe_looseId==1)'
   'passtight'   : " abs(Probe_dxy) < 0.05 && abs(Probe_dz) < 0.1 && Probe_miniPFRelIso_all<0.4 && Probe_sip3d < 8 && Probe_looseId==1"
}
#baseOutDir = 'results_v12/ele_tight_altMC/'
#baseOutDir = 'results_v12_preEE_mva09/ele_recototight/'
baseOutDir = 'results_mvaTTH_2023_RECO_fullversion/muons/'
...
import etc.inputs.tnpSampleDef_mvaTTH_muo as tnpSamples
...
cutBase = "TnP_trigger && Tag_conept > 30  && abs(Tag_eta) < 2.4 && Tag_jetRelIso < 0.15 && Probe_charge*Tag_charge < 0 && abs(Tag_dxy) < 0.05 && abs(Tag_dz) < 0.1 && Tag_isGenMatched && Probe_isGenMatched == 1 && Probe_conept > 10 && abs(Probe_eta) < 2.4 && Probe_isGlobal==1"
...
tnpParNomFit = [
    ...
    #"meanF[0.0,-5.0,5.0]","sigmaF[0.4,0.2,5.0]",
    #"acmsP[60.,50.,100.]","betaP[0.05,0.01,0.08]","gammaP[0.1, -2, 2]","peakP[90.0]",
    #"acmsF[60.,50.,90.]","betaF[0.05,0.001,0.08]","gammaF[0.1, -1, 2]","peakF[90.0]",
...
tnpParAltBkgFit = [
    ...
    "alphaF[0.,-5.,5.]",
    #bin01
    #"meanP[-0.4,-1.0,1.0]","sigmaP[1.8,0.1,5.0]",
    #"meanF[0.3,-1.0,1.0]","sigmaF[1.0,0.1,5.0]",
    #"alphaP[-0.8,-1.,2.]",
    #"alphaF[-0.6,-1.,1.]",
